...
```json
{
  "mcpServers": {
    "gridea-pro": {
      "command": "/usr/local/bin/gridea-pro-mcp",
      "env": {
...
2. 避免和 Wails 打包产生冲突
3. 可以通过 `go build -o gridea-pro-mcp ./backend/cmd/mcp` 单独构建
...
MCP 协议支持两种主要的传输方式：
1.  **stdio（标准输入输出）**：Claude Desktop 作为一个父进程，直接启动 `gridea-pro-mcp` 二进制文件，通过管道（Pipe）进行通信。
    *   **特点**：简单、零网络开销、随 Claude 启动/关闭。
    *   **适用**：本地桌面应用（目前的 Gridea Pro 场景）。
2.  **SSE（基于 HTTP）**：`gridea-pro-mcp` 作为一个独立的 Web 服务器运行（例如监听 8080 端口），Claude 通过 HTTP 协议连接。
...
```bash
# 启动服务并监听端口
./gridea-pro-mcp --transport sse --port 8080
```
...
go build -o gridea-pro-mcp ./backend/cmd/mcp
...
> **关键点**：`command` 必须指向 `gridea-pro-mcp` 这个二进制文件的**真实绝对路径**。如果不确定，可以在终端找到它然后输入 `pwd` 查看。
...
- [x] MCP 启动方式 → **独立二进制 (`gridea-pro-mcp`)**
- [x] 首期 Tool 范围 → **全量实现**
- [x] 是否加入 Prompts → **需要**
